...
Authors  :: Adan Yusseff Dominguez Ruiz
Modified from work by:
Vikash Kumar (vikashplus@gmail.com), Vittorio Caggiano (caggiano@gmail.com), Pierre Schumacher (schumacherpier@gmail.com), Cameron Berg (cam.h.berg@gmail.com)
================================================= """
...
from myosuite.envs.myo.myobase.walk_v1 import WalkEnvV0
...
class ChaseAmputeeEnvV0(WalkEnvV0):
    ...
    def _setup(self,
        ...
        print("Initialization of Opponent")
    ...
    def _get_fallen_condition(self):
        ...
        if self.terrain == 'FLAT':
            ...
            foot_l = self.sim.data.body('l_Ankle_Assembly').xpos
...
class AmputeeChallengeV0(WalkEnvV0):
    #TODO
    #Modify movement on motor foot. Compare with shadow hand environment to modify values.
    #Modify Motor_force Function
    #Modify Motor_lenghts Function
    #Track movement of the natural foot and give rewards for similar movement.
    #Modify specific floors
    #Make sure target is placed correctly
    #Rotation in rewards must point to the target direction
    ...
    def __init__(self, model_path, obsd_model_path=None, seed=None, **kwargs):
        # This flag needs to be here to prevent the simulation from starting in a done state
        # Before setting the key_frames, the model and target will be in the cartesian position,
        # causing the step() function to evaluate the initialization as "done".
        self.startFlag = False
    ...
    def _setup(self,
        ...
        self.target = ChosenTarget(sim=self.sim,
                                      rng=self.np_random,
                                      )
        self.win_distance = win_distance
        self.grf_sensor_names = ['r_foot', 'r_toes', 'l_prosth']
        ...
        self.init_qvel[:] = 0.0
        self.startFlag = True
        self.target.dt = self.sim.model.opt.timestep * self.frame_skip
    ...
    def get_obs_dict(self, sim):
        ...
        # exteroception
        obs_dict['target_pose'] = self.target.get_target_pose()[:].copy()
        obs_dict['model_root_pos'] = sim.data.qpos[:2].copy()
    ...
    def get_reward_dict(self, obs_dict):
        ...
        # Example reward!
        distance = np.linalg.norm(obs_dict['model_root_pos'][...,:2] - obs_dict['target_pose'][...,:2])
    ...
    def step(self, *args, **kwargs):
        self.target.update_target_state()
        obs, reward, done, info = super().step(*args, **kwargs)
        return obs, reward, done, info
    ...
    def reset(self):
        ...
        self.robot.sync_sims(self.sim, self.sim_obsd)
        obs = super(WalkEnvV0, self).reset(reset_qpos=qpos, reset_qvel=qvel)
        self.target.reset_target(player_task=self.current_task.name, rng=self.np_random)
        self.sim.forward()
        return obs
    ...
    def _win_condition(self):
        root_pos = self.sim.data.body('pelvis').xpos[:2]
        target_pos = self.obs_dict['target_pose'][..., :2]
        if np.linalg.norm(root_pos - target_pos) <= self.win_distance and self.startFlag:
            return 1
        return 0
